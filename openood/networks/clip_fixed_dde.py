...
# same indices with imagenet_XX_wnids
...
def get_class_names(dataset):
    if dataset == 'imagenet' or dataset == 'imagenetv2'  or dataset == 'imagenets':
        ...
        return imagenet10_classes
    elif dataset == 'imagenet_20':
        return imagenet20_classes
    elif dataset == 'imagenet_100':
        return imagenet100_classes
    elif dataset == 'waterbird':
        return waterbird_classes
...
def get_templates(text_prompt):
    if 'simple' == text_prompt:
        return simple_imagenet_template
    elif 'tip' == text_prompt:
        return tip_imagenet_templates
    elif 'vanilla' == text_prompt:
        return ['{}.']
    elif 'nice' == text_prompt:
        return ['The nice {}.']
    elif 'good' == text_prompt:
        return ['a good photo of the {}.']
    elif 'bad' == text_prompt:
        return ['a bad photo of the {}.']
    elif 'small' == text_prompt:
        return ['a photo of the small {}.']
    elif 'large' == text_prompt:
        return ['a photo of the large {}.']
    elif 'full' == text_prompt:
...
def get_text_features(model, dataset, text_prompt):
    ...
    text_features = torch.cat((text_features, mean_text), dim=1)
...
class FixedCLIP_OODPrompt(nn.Module):
    ...
    def forward(self, x, return_feat=False):
        ...
        if return_feat:
            return image_features, self.text_features.t(), logit_scale
        else:
            logits = logit_scale * image_features @ self.text_features 
            return logits


def get_selected_ood_text_list(dataset='imagenet', total_ood_num=1000):
...
class FixedCLIP_DDE(nn.Module):
    def __init__(self, cfg):
        ...
        uniform_noise_image = torch.rand((3, 3, 224, 224), dtype=torch.float).cuda()
        poisson_noise_image = torch.poisson(5.0 * torch.ones((3, 3, 224, 224))).cuda()
        poisson_noise_image = (poisson_noise_image - poisson_noise_image.min()) / (poisson_noise_image.max() - poisson_noise_image.min())
        gamma_noise = torch.distributions.Gamma(2, 1).sample((3,3,224,224)).cuda()
        gamma_noise = (gamma_noise - gamma_noise.min()) / (gamma_noise.max() - gamma_noise.min())
        salt_and_pepper_noise = torch.rand((3, 3, 224, 224), dtype=torch.float).cuda()
        salt_and_pepper_noise[salt_and_pepper_noise >= 0.5] = 1.0
        salt_and_pepper_noise[salt_and_pepper_noise < 0.5] = 0
        all_noise = torch.cat((gaussian_noise_image, uniform_noise_image, poisson_noise_image, gamma_noise, salt_and_pepper_noise), dim=0) ## 15*3*224*224
        with torch.no_grad():
            noise_image_features = self.model.encode_image(all_noise)
            self.noise_image_features = noise_image_features / noise_image_features.norm(dim=-1, keepdim=True)
