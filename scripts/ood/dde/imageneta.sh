...
do
    for hat_M in 100 #  2000 5000 10000 20000 50000 100000 200000 250000   ## activated-aware scores, 0 means activation-agnostics  ptis tisp  ###part_of_speech_tagging, neglabel_wordnet  fixedclip_negoodprompt_fullcorpus
    do
        CUDA_VISIBLE_DEVICES=3 python main.py \
        --config configs/datasets/imagenet/imagenet_cross_domain_imageneta.yml \
        configs/networks/fixed_clip.yml \
        configs/pipelines/test/test_fsood.yml \
        configs/preprocessors/base_preprocessor.yml \
        configs/postprocessors/dde.yml \
        --dataset.train.batch_size 16 \
        --dataset.train.few_shot 0 \
        --dataset.num_classes $((ood_number + 200)) \
        --ood_dataset.num_classes $((ood_number + 200)) \
        --evaluator.name ood_clip_dde \
        --evaluator.ood_scheme ood \
        --network.name fixedclip_dde \
        ...
        --network.backbone.cls_text_prompt ${cls_text_prompt} \
        --network.backbone.text_center True \
        --network.pretrained False \
        --postprocessor.APS_mode False \
        --postprocessor.name dde \
        ...
        --postprocessor.postprocessor_args.dota_rho ${dota_rho}  \
        ...
        --postprocessor.postprocessor_args.hat_M ${hat_M}  \
        --postprocessor.postprocessor_args.cossim False  \
        --num_gpus 1 --num_workers 8 \
        --merge_option merge \
        --output_dir ./aaneg_analyses_final_seed/ \
        --mark ${in_score}_alpha${alpha}_beta${beta}_gamma${gamma}_neg10k_ood_number_${ood_number}_random_${random_permute}_thres${thres}_gap${gap}_gsize${group_size}_seed${seed}
    done
done